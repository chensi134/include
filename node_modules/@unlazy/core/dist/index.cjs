...
const blurhash = require('./shared/core.CheZc8bg.cjs');
const thumbhash = require('./thumbhash.cjs');
require('./shared/core.CJlWKc-B.cjs');
...
function lazyLoad(selectorsOrElements = 'img[loading="lazy"]', {
  ...
  for (const image of blurhash.toElementArray(selectorsOrElements)) {
    ...
    if (blurhash.isCrawler || !blurhash.isLazyLoadingSupported) {
...
function updateSizesAttribute(element, options) {
  ...
  if (options?.updateOnResize) {
    if (!resizeElementStore.has(element)) {
      const debounceResize = blurhash.debounce(() => updateSizesAttribute(element), 500);
...
exports.debounce = blurhash.debounce;
exports.getScaledDimensions = blurhash.getScaledDimensions;
exports.isCrawler = blurhash.isCrawler;
exports.isLazyLoadingSupported = blurhash.isLazyLoadingSupported;
exports.isSSR = blurhash.isSSR;
exports.toElementArray = blurhash.toElementArray;
